...
class ChatView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if you_chat_user and Chat.objects.filter(users__in=[me_chat_user, you_chat_user]).exists():
            chat_room = Chat.objects.filter(users__in=[me_chat_user, you_chat_user]).first().chat_room
        elif you_chat_user and not Chat.objects.filter(users__in=[me_chat_user, you_chat_user]).exists():
        ...
        kwargs["peoples"] = User.objects.filter(chats__users__in=[me_chat_user]).exclude(id=me_chat_user.id).distinct()
